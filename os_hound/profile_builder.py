...
class ProfileBuilder:
    ...
    def build_profile(self):
        ...
        # SEQ
        diff, gcd = self.methods.tcp_isn_gcd(self.responses["SYN"])
        isr, seq_rates = self.methods.tcp_isn_isr(diff)
        os_dict["SEQ"]["SP"] = self.methods.tcp_isn_sp(seq_rates, gcd)
        os_dict["SEQ"]["GCD"] = gcd
        os_dict["SEQ"]["ISR"] = isr
        os_dict["SEQ"]["TI"] = self.methods.ip_id_sequence(self.responses["SYN"], "TI")
        os_dict["SEQ"]["CI"] = self.methods.ip_id_sequence(self.responses["SYN"], "CI")
        os_dict["SEQ"]["II"] = self.methods.ip_id_sequence(self.responses["SYN"], "II")

        opt_ii = ["RI", "BI", "I"]
        if os_dict["SEQ"]["TI"] == os_dict["SEQ"]["II"] and os_dict["SEQ"]["II"] in opt_ii:
            os_dict["SEQ"]["SS"] = self.methods.shared_ip_id(self.responses["SYN"], self.responses["IE"])
        os_dict["SEQ"]["TS"] = self.methods.calculate_ts(self.responses["SYN"])

        # OPS
        op_list = self.methods.extract_tcp_options(self.responses["SYN"])
        op_counter = 1
        for op in op_list:
        ...
        # WIN
        ws_list = self.methods.extract_tcp_window_size(self.responses["SYN"])
        ws_counter = 1
        for ws in ws_list:
        ...
        # ECN
        os_dict["ECN"]["R"] = self.methods.check_responsiveness("ECN", self.responses["ECN"])
        os_dict["ECN"]["DF"] = self.methods.check_dont_fragment_bit(self.responses["ECN"])
        os_dict["ECN"]["T"] = self.methods.compute_initial_ttl(self.responses["ECN"], self.responses["U1"])
        os_dict["ECN"]["TG"] = self.methods.ttl_guess_test(self.responses["ECN"])
        os_dict["ECN"]["W"] = self.methods.extract_tcp_window_size(self.responses["ECN"])
        os_dict["ECN"]["O"] = self.methods.extract_tcp_options(self.responses["ECN"])
        os_dict["ECN"]["CC"] = self.methods.congestion_control_test(self.responses["ECN"])
        os_dict["ECN"]["Q"] = self.methods.check_tcp_quirks(self.responses["ECN"])

        # T1
        t1_seq_num = self.responses["SYN"][0][TCP].seq
        os_dict["T1"]["R"] = self.methods.check_responsiveness("SYN", self.responses["SYN"][0])
        os_dict["T1"]["DF"] = self.methods.check_dont_fragment_bit(self.responses["SYN"][0])
        os_dict["T1"]["T"] = self.methods.compute_initial_ttl(self.responses["SYN"][0], self.responses["U1"])
        os_dict["T1"]["TG"] = self.methods.ttl_guess_test(self.responses["SYN"][0])
        os_dict["T1"]["S"] = self.methods.sequence_test(self.responses["SYN"][0], t1_seq_num)
        os_dict["T1"]["A"] = self.methods.ack_test(self.responses["SYN"][0], t1_seq_num)
        os_dict["T1"]["F"] = self.methods.extract_tcp_flags(self.responses["SYN"][0])
        os_dict["T1"]["RD"] = self.methods.get_rst_data_checksum(self.responses["SYN"][0])
        os_dict["T1"]["Q"] = self.methods.check_tcp_quirks(self.responses["SYN"][0])
        ...
        # U1
        os_dict["U1"]["R"] = self.methods.check_responsiveness("U1", self.responses["U1"])
        os_dict["U1"]["DF"] = self.methods.check_dont_fragment_bit(self.responses["U1"])
        os_dict["U1"]["T"] = self.methods.compute_initial_ttl(self.responses["U1"], self.responses["U1"])
        os_dict["U1"]["TG"] = self.methods.ttl_guess_test(self.responses["U1"])
        os_dict["U1"]["IPL"] = self.methods.get_ip_total_length(self.responses["U1"])
        os_dict["U1"]["UN"] = self.methods.check_icmp_unused_field(self.responses["U1"])
        os_dict["U1"]["RIPL"] = self.methods.check_returned_ip_length(self.responses["U1"])
        os_dict["U1"]["RID"] = self.methods.check_returned_ip_id(self.responses["U1"])
        os_dict["U1"]["RIPCK"] = self.methods.check_returned_ip_checksum(self.responses["U1"])
        os_dict["U1"]["RUCK"] = self.methods.check_returned_udp_checksum(self.responses["U1"])
        os_dict["U1"]["RUD"] = self.methods.check_returned_udp_data_integrity(self.responses["U1"])

        # IE
        r1 = self.methods.check_responsiveness("IE", self.responses["IE"][0])
        r2 = self.methods.check_responsiveness("IE", self.responses["IE"][1])
        if r1 and r2:
            os_dict["IE"]["R"] = "Y"
        else:
            os_dict["IE"]["R"] = "N"

        os_dict["IE"]["DFI"] = self.methods.dfi_test_value(self.responses["IE"])
        os_dict["IE"]["T"] = self.methods.compute_initial_ttl(self.responses["IE"][0], self.responses["U1"])
